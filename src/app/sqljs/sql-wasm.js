...
function dbConnection(){
 ...
 var createandopen=0;
 var odb;
 dbConnection.prototype.createDb=function(){
   this.myDb=new SQLITE.Database();
   this.state=1;
   createandopen=1;
   return this.myDb;
 }

 dbConnection.prototype.loadDb=function(dburl){
   const xhr = new XMLHttpRequest();
   xhr.open('GET', dburl, true);
   xhr.responseType = 'arraybuffer';
   xhr.onload = e => {
     const uInt8Array = new Uint8Array(xhr.response);
     SQLITEDB = new SQLITE.Database(uInt8Array);

     };
   xhr.send();
 }
 dbConnection.prototype.setDb=function(){
   if(this.state==0 && SQLITEDB !=null){
     this.myDb=SQLITEDB;
     this.state=1;
   }
 }
 dbConnection.prototype.open=function(db){
   if(this.state==0){
     this.myDb=db;
     this.state=1;
     createandopen=0;
 ...
 dbConnection.prototype.execute=function(sql){
   if(this.state==0 && SQLITEDB !=null){
     this.myDb=SQLITEDB;
     this.state=1;
   }
   if(this.state==1){
     return this.myDb.run(sql).getRowsModified();
   }
 }
 dbConnection.prototype.exec=function(sql){
   if(this.state==0 && SQLITEDB !=null){
     this.myDb=SQLITEDB;
     this.state=1;
   }
   return this.myDb.exec(sql);
 }
 dbConnection.prototype.save=function(filename){
   var fname="jsql.db";
   if(filename!=undefined)
     fname=filename+".db";
   if(this.myDb!=null){
       const binaryArray = this.myDb.export();
