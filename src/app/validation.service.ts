...
export class ValidationService {
    ...
    static getErrorMessage(validatorName: string, validatorValue?: any) {
        let messages:any = {
            'required': 'Required',
            'email':'Invalid Email',
            'invalidPassword': 'Invalid password. Password must be at least 8 characters long, and must contain a uppercase, number and a special character.',
    ...
    static passwordValidator(control:any) {
        ...
        if (control.value.match(/^(?=.*[0-9])(?=.*[!@#$%^&*])(?=.*[A-Z]).{8,}$/)) {
            return null;
        } else {
