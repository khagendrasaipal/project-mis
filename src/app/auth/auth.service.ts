            ...
            this.http.post(this.path + "/login", { username: username, password: password })
                .subscribe({next:(response:any) => {
                    ...
                    if (isToken) {
                        ...
                        localStorage.setItem('currentUser', JSON.stringify({ username: username, name: data.name,permid:data.permid, token: data.token, bank: data.bank, branch: data.branch,dlgid:data.dlgid }));
                    }
